...
```{r}
demo_raw <- read.csv(paste0(workingdir, "/Task Switching_Demographics_Data.csv"), skip = 1) %>%
  slice(-1) %>%
  select(-c(Response.Type,IP.Address, Recipient.Last.Name:Distribution.Channel))
colnames(demo_raw)[9:14] <- c("Gender.Pick", "Gender.Text","Age","Race.Pick","Race,Text","Eng.First")
