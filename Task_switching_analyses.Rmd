...
#Import current data
  task_switching_raw <- read.csv(paste0(workingdir, "/task-switching-replication-recoded.csv"))
...
#Drop participants w/<=80% acc
  #8, 49, 44, 2, 15, 51
     ...
     task_switching_raw3 <- task_switching_raw2 %>% 
                              filter(!(participant %in% low_acc_subs$participant))
...
#Convert data to wide format 
  ...
  ts_acc_mean$congruantTrialType <- as.factor(ts_acc_mean$congruantTrialType)
...
exp1_anova <- ezANOVA(ts_acc_mean, 
                      dv = Accuracy, 
                      wid = participant, 
                      within = .(posture, congruantTrialType, switchTrialType), 
                      type = 3, 
                      detailed = TRUE,
                      return_aov = T
                      )
...
#                      Error(participant/posture*congruantTrialType*switchTrialType),
#                      ts_acc_mean)
# #str(ts_acc_mean)
